(() => ({
  ...
  jsx: (() => {
    const { env, useText } = B;
    const { imgUrl, imgAlt, imgType, width, height} = options;
    const imgSrc = useText(imgUrl);
    const variable = imgUrl && imgUrl.findIndex(v => v.name) !== -1;
    const variableDev = env === 'dev' && ((variable || !imgSrc) && imgType === 'None');

    const w = width || 250;
    const h = height || 250;
    
    const src = imgType ? `https://placeimg.com/${w}/${h}/${imgType}/grayscale` : imgSrc;
    ...
    return (
      ...
      >
        {variableDev ? (
          ...
          <img className={classes.media} src={src} alt={useText(imgAlt)} />
        )}
      </figure>
    );
  })(),
